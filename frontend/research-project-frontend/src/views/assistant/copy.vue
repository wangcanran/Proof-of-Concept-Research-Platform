    ...
    <div class="dashboard-content">
      ...
      <aside class="sidebar" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
        ...
        <nav class="sidebar-nav">
          ...
          <div class="nav-section">
            <h4 v-if="!sidebarCollapsed" class="nav-section-title">项目审核</h4>
            <router-link to="/audit/projects" class="nav-link" active-class="active">
              <span class="nav-icon">📋</span>
            ...
            <router-link to="/assistant/applications" class="nav-link" active-class="active">
              <span class="nav-icon">📝</span>
              <span v-if="!sidebarCollapsed" class="nav-text">申请管理</span>
            </router-link>
          </div>
          ...
          <div class="nav-section">
            <h4 v-if="!sidebarCollapsed" class="nav-section-title">辅助功能</h4>
            <router-link to="/audit/tasks" class="nav-link" active-class="active">
              <span class="nav-icon">✅</span>
              <span v-if="!sidebarCollapsed" class="nav-text">任务列表</span>
            </router-link>
          </div>
        </nav>
                ...
                >
                  <div class="application-header">
                    <span class="application-id"
                      >项目#{{ application.project_code || application.id.substring(0, 8) }}</span
                    >
...
const goToPendingProjects = () => {
  router.push('/audit/projects?status=submitted')
}

const goToPendingFunding = () => {
  router.push('/audit/funding?status=submitted')
}

const goToPendingExpenditures = () => {
  router.push('/audit/expenditures?status=submitted')
}

const goToPendingAchievements = () => {
  router.push('/audit/achievements?status=submitted')
}
...
const loadOverviewData = async () => {
  try {
    const response = await request.get('/api/assistant/dashboard/overview')
...
const loadPendingApplications = async () => {
  try {
    const response = await request.get('/api/assistant/applications/pending', {
      params: { limit: 5 },
    })
    if (response.success && response.data) {
...
const loadProjectStats = async () => {
  try {
    const response = await request.get('/api/assistant/stats/projects')
    if (response.success && response.data) {
      projectStats.value = response.data.map((stat: any) => ({
        ...stat,
...
const loadRecentActivities = async () => {
  try {
    const response = await request.get('/api/assistant/activities/recent', {
      params: { limit: 5 },
    })
    if (response.success && response.data) {
...
const loadRecentUsers = async () => {
  try {
    const response = await request.get('/api/assistant/users/recent', {
      params: { limit: 5 },
    })
    if (response.success && response.data) {
...
const loadPendingTasksData = async () => {
  try {
    const response = await request.get('/api/assistant/tasks/pending')
    if (response.success && response.data) {
      pendingTasks.value = {
...
const showMockData = () => {
  ...
  projectStats.value = [
    { status: 'draft', label: '草稿', count: 5, percentage: 12.5, color: '#f5f5f5' },
    { status: 'submitted', label: '已提交', count: 8, percentage: 20, color: '#e6f7ff' },
    { status: 'under_review', label: '审核中', count: 10, percentage: 25, color: '#fff7e6' },
    { status: 'in_progress', label: '进行中', count: 15, percentage: 37.5, color: '#1890ff' },
  ...
  recentActivities.value = [
    {
      ...
      icon: '📝',
      color: '#e6f7ff',
      description: '您审核了"人工智能在医疗诊断中的应用研究"项目',
      time: '2小时前',
    },
    {
      id: 'act_002',
  ...
  notifications.value = [
    {
      ...
      icon: '📢',
      title: '系统维护通知',
      description: '本周末（2月3-4日）系统将进行维护，请提前保存工作',
      time: '今天 09:00',
      read: false,
    },
    {
...
onMounted(() => {
  ...
  loadUserInfo().then(() => {
    ...
    if (userRole?.toLowerCase() !== 'assistant') {
      console.warn(`⚠️ 警告：当前用户角色 "${userRole}" 不匹配科研助理角色`)
...
<style scoped>
/* 复用AdminDashboard的样式基础，添加科研助理特有样式 */
.assistant-dashboard {
  min-height: 100vh;
  background: #f5f7fa;
  font-family:
...
.welcome-card.assistant-banner .illustration-icon {
  color: rgba(255, 255, 255, 0.8);
}
