  ...
  <div class="review-list-page">
    <div class="page-header">
      <div class="header-left">
        ...
        <h1>企业服务成果审核</h1>
        <div class="header-subtitle">审核本人负责项目下的技术合作成果及资质认定成果</div>
      </div>
    </div>
          ...
          <div v-for="item in filteredList" :key="item.id" class="request-card">
            ...
            <div class="card-body" @click="goDetail(item.id)">
              <div class="card-info">
                ...
                <span class="info-value">{{ typeLabel(item.achievement_type) }}</span>
              </div>
              <div class="card-info">
              ...
              <div class="card-info">
                <span class="info-label">{{ enterpriseLabel(item.achievement_type) }}</span>
...
interface ReviewItem {
  ...
  projects?: Array<{ project_title?: string; title?: string; project_code?: string }>
}
...
const statusTabs = computed(() => [
  { value: 'all', label: '全部', count: allList.value.length },
  {
    value: 'submitted',
    label: '待审核',
    count: allList.value.filter((i) => i.status === 'submitted').length,
  },
  {
    value: 'verified',
    label: '已核实',
    count: allList.value.filter((i) => i.status === 'verified').length,
  },
  {
    value: 'rejected',
    label: '已驳回',
    count: allList.value.filter((i) => i.status === 'rejected').length,
  },
])
...
const filteredList = computed(() => {
  ...
  if (currentTab.value !== 'all') {
    list = list.filter((i) => i.status === currentTab.value)
  }
  return list
})

function typeLabel(t: string) {
  const found = ENTERPRISE_ACHIEVEMENT_TYPES.find((x) => x.value === t)
...
function projectTitles(item: ReviewItem) {
  const projects = item.projects || []
  if (!projects.length) return '—'
  return projects.map((p) => p.project_title || p.title || '-').join('、')
}

function projectCodes(item: ReviewItem) {
  const projects = item.projects || []
  if (!projects.length) return '—'
...
function enterpriseLabel(type: string) {
  return type === 'qualification_certification' ? '获资质企业' : '服务企业'
}

function enterpriseName(item: ReviewItem) {
  if (item.achievement_type === 'qualification_certification') {
    return item.qualified_enterprise || '—'
  }
  return item.service_enterprise || '—'
}

function primaryProjectTitle(item: ReviewItem) {
  const projects = item.projects || []
  if (!projects.length) return '—'
  return projects[0].project_title || projects[0].title || '—'
}

function getCardTitle(item: ReviewItem) {
...
async function loadList() {
  ...
  loading.value = true
  try {
    const res = await assistantEnterpriseServiceAPI.list({
      page: 1,
      pageSize: 200,
...
function goDetail(id: string) {
  router.push(`/enterprise-service-achievements/${id}/detail`)
}

function goReview(id: string) {
  router.push(`/assistant/enterprise-service-achievements/${id}/review`)
}
