...
const loadReviewData = async () => {
  ...
  try {
    ...
    // 情况1：有 reviewId，加载评审详情
    if (route.params.id) {
      reviewId.value = route.params.id
    ...
    // 情况2：有 projectId，加载项目信息准备评审
    else if (route.query.projectId) {
      projectId.value = route.query.projectId as string
      console.log('📥 加载项目信息，ID:', projectId.value)

      // 加载项目信息...
    }
    // 情况3：都没有，应该显示待评审项目列表
    else {
      console.log('📋 显示待评审项目列表')

      // 这里应该显示列表，而不是错误
      // 暂时重定向到仪表板或显示空状态
      ElMessage.info('请从待评审项目列表中选择项目进行评审')
