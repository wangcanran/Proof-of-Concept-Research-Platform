...
-- 说明: 科研项目管理系统完整建表脚本
--
-- 来源: research_system_db.sql + database 目录下全部增量脚本，
--       并与本地 research_system 库结构核对。
--
-- 用法（Windows 请指定 utf8mb4）:
--   mysql -u root -p --default-character-set=utf8mb4 < research_system_db_0626.sql
-- 从零重建（会删库）:
...
CREATE TABLE IF NOT EXISTS `ExpertProfile` (
    ...
    `keywords` VARCHAR(500) DEFAULT NULL COMMENT '专业关键词（大模型根据 expertise_description 自动提取，逗号分隔，用于专家检索）',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
...
-- 8. 项目与领域多选关联（每条对应 ResearchDomain 中一项）
...
CREATE TABLE IF NOT EXISTS `ProjectAttachment` (
    ...
    `type` ENUM('image', 'video', 'audio', 'attachment') NOT NULL COMMENT '分类',
...
-- 18. 项目经费申请附件表（全流程附件：申请附件/反馈附件/成果附件）
...
CREATE TABLE IF NOT EXISTS `IncubationAchievementRecord` (
    ...
    `reviewed_by` VARCHAR(36) DEFAULT NULL COMMENT '审批人（项目经理）',
...
-- 25. 产业资源与项目关联表（推送 + 认领）declined状态没有用
CREATE TABLE IF NOT EXISTS `ProjectEnterpriseDemand` (
    `id` VARCHAR(36) PRIMARY KEY,
    `project_id` VARCHAR(36) NOT NULL COMMENT '项目ID',
    `demand_id` VARCHAR(36) NOT NULL COMMENT '企业需求ID',
    `pushed_by` VARCHAR(36) DEFAULT NULL COMMENT '推送人（项目经理）；申请人主动申请时为空',
