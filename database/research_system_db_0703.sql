...
-- 说明: 科研项目管理系统完整建表脚本
--
-- 来源: research_system_db_0627.sql + 产业资源库相关增量
--       (add_industry_partner_library.sql、
--        add_industry_partner_connection_requests.sql、
--        unify_industry_partner_engagement.sql 中的表结构部分)
--
-- 用法（Windows 请指定 utf8mb4）:
--   mysql -u root -p --default-character-set=utf8mb4 < research_system_db_0703.sql
-- 从零重建（会删库）:
--   mysql -u root -p --default-character-set=utf8mb4 < init.sql
--   mysql -u root -p --default-character-set=utf8mb4 < research_system_db_0703.sql
--   mysql -u root -p --default-character-set=utf8mb4 < sample_data.sql
--
-- 从 0627 升级（已有库）请执行增量脚本，勿重复执行本全量脚本中的 CREATE。
-- =============================================
...
CREATE TABLE IF NOT EXISTS `IndustryResource` (
    ...
    `source_partner_id` VARCHAR(36) DEFAULT NULL COMMENT '需求来源（产业资源库机构）',
...
-- 25. 孵化服务申请-产业资源库机构分配（历史/辅助表，承接关系以对接申请表为准）
...
-- 26. 产业资源库对接/承接记录（项目方申请确认 与 项目经理服务分配 共用）
CREATE TABLE IF NOT EXISTS `IndustryPartnerConnectionRequest` (
    ...
    `source` ENUM('applicant_apply', 'pm_service_assign') NOT NULL DEFAULT 'applicant_apply'
        COMMENT '承接来源：项目方申请 / 项目经理服务分配',
    `incubation_progress_id` VARCHAR(36) DEFAULT NULL COMMENT '服务申请ID（项目经理分配时关联）',
    ...
    INDEX idx_incubation_progress (`incubation_progress_id`),
    INDEX idx_handled_by (`handled_by`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='产业资源库对接/承接记录';
...
CREATE TABLE IF NOT EXISTS `TransformationAchievement` (
    ...
    `industry_partner_id` VARCHAR(36) DEFAULT NULL COMMENT '承接方（产业资源库机构）',
    ...
    INDEX idx_industry_partner (`industry_partner_id`)
